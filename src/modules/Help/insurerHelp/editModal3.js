...
import { success, clear, CreateMaterQuery, sampleFile, clearSampleURL } from "../help.slice";

const EditModal = ({ show, onHide, userType, currentUser }) => {
    const dispatch = useDispatch();
    const { Querysuccess, error, sampleURL } = useSelector((state) => state.help);
    ...
    const onSubmit = (data) => {
        if (data.file[0]) {
            ...
            dispatch(CreateMaterQuery(formData));
        }
        else {
        ...
        >
            <Form onSubmit={handleSubmit(onSubmit)}>
                <Modal.Header closeButton>
                    <Modal.Title id="contained-modal-title-vcenter">Add Query Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Row>
