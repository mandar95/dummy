...
import { getPoliciesData, selectPolicies } from "./MyPolicy.slice";
...
const PolicyTable = (props) => {
  ...
  const getTableData = useSelector(selectPolicies);
  const { globalTheme: theme } = useSelector((state) => state.theme);
  //states
  const [tableData, setTableData] = useState([]);
  useEffect(() => {
    if (!_.isEmpty(getTableData?.data?.data)) {
    ...
    <tr key={index + 'policy-table-852'}>
      ...
      {/* <th>
        <button
          ...
          onClick={() => window.open(TableArray?.redirect_url)}
        >
      ...
      <th>
        <button
          ...
          onClick={() => window.open(TableArray?.doc_path)}
        >
          <i className="ti-file" />
        </button>
      </th>
