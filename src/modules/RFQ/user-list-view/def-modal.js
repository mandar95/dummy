...
export const EditModal = ({ show, onHide, Data, ic, brokerId }) => {
	...
	useEffect(() => {
		if (Data.status) setValue("status", Data.status === "Open" ? 0 : 1);
	...
	const onSubmit = (data) => {
		...
		!Data.deficiency_type
			? dispatch(addDeficiency(formData))
			: dispatch(updateDeficiency(formData, Data.id));
	};
						...
						{Data.deficiency_type ||
							...
							<Col md={6} lg={5} xl={4} sm={12}>
								<Controller
									...
									name="status"
									control={control}
									defaultValue={Data.status === "Open" ? 0 : 1}
								/>
							</Col>
						) : (
							<>
