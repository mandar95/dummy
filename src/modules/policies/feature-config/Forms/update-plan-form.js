...
const UpdatePlanForm = ({ Controller, control, Rater, RaterType, rater_type_id, SIType,
  is_policy_level, SI, suminsuredDataTwo, AddSIValue, SIValue, RemoveSIValue, FetchSumInsured, Data,
  si_type, policyId, setSumInsuredDataTwo }) => {
  return (
                ...
                <>
                  <Col md={6} lg={5} xl={4} sm={12}>
                    <Controller
                      ...
                      defaultValue={() => {
                        if (["grade_with_contruct", "grade"].includes(SIType) && typeof (Data) === "object") {
                          let a = SI.filter(item => Number(item.grade_id) === Number(Data.grade_id));
                          FetchSumInsured({
                            value: si_type || a[0]?.id,
                            policy_id: policyId,
                            rater_type_id: Rater.length > 0 ? rater_type_id : RaterType
                          }, setSumInsuredDataTwo);
                          return a[0]?.id;
                        } else if (["designation_with_construct", "designation"].includes(SIType) && typeof (Data) === "object") {
                          let a = SI.filter(item => Number(item.designation_id) === Number(Data.designation_id));
                          FetchSumInsured({
                            value: si_type || a[0]?.id,
                            policy_id: policyId,
                            rater_type_id: Rater.length > 0 ? rater_type_id : RaterType
                          ...
                          return a[0]?.id;
                        } else if (["age"].includes(SIType) && typeof (Data) === "object") {
                          let a = SI.filter(item => _.isEqual(item.age, Data.age));
                          FetchSumInsured({
                            value: si_type || a[0]?.id,
                            policy_id: policyId,
                            rater_type_id: Rater.length > 0 ? rater_type_id : RaterType
                          ...
                          return a[0]?.id;
                        } else if (["state"].includes(SIType) && typeof (Data) === "object") {
                          let a = SI.filter(item => _.isEqual(item.state_id, Data.state_id));
                          FetchSumInsured({
                            value: si_type || a[0]?.id,
                            policy_id: policyId,
                            rater_type_id: Rater.length > 0 ? rater_type_id : RaterType
