...
import { PolicyWise, StatusWise, ActivityWise, DocumentWise } from "./dashboard_insurer.slice"
...
const EditModal = ({ show, userType, onHide, id }) => {
    ...
    const onSubmit = ({ till_date, from_date }) => {
        ...
        switch (parseInt(id)) {
            case 1:
                (currentUser?.ic_user_type_id === 1 ? dispatch(PolicyWise(resp)) : dispatch(DocumentWise(resp)))
                break;
            case 2:
