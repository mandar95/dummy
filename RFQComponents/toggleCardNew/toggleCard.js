...
const ToggleCard = ({
	...
	const activeCardAction = (id) => {
		const valueCheck = activeCard.filter((item) => parseInt(item) === id);
		if (!_.isEmpty(valueCheck) && valueCheck[0] !== 1) {
	...
	// }, [data]);
	useEffect(() => {
		const activeValueArray = (typeof activeValue) === 'string' ? JSON.parse(activeValue) : activeValue
		if (typeof activeValueArray === 'object') {
			setActiveCard(activeValueArray)
		}
